...
{
    /// <summary>
    /// ChineseCalendarInfo C# 阳历转阴历 的摘要说明。
    /// </summary>
   public  class ChineseCalendarInfo
    {
        ...
        /// 确保有两位数字，不足前补0
        /// </summary>
        /// <param name="Int">传入的整数</param>
        /// <returns>返回两位字符</returns>
        public string strInt(int Int)
        {
       ...
       public string LunarDate
       {
           get
           {
               return Convert.ToString(m_LunarYear) + "-" + strInt(m_LunarMonth) + "-" + strInt(m_LunarDay);
               //DateTime.ParseExact(inValue.ToString(),"yyyyMMdd",System.Globalization.CultureInfo.CurrentCulture).
               //DateTime dtTmp 1= Convert.ToString(m_LunarYear.ToString() + "-" + m_LunarMonth.ToString() + "-" + m_LunarDay.ToString() +" 0:00:00"));
               //return DateTime.ParseExact(dtStr ,"yyyy-MM-dd",System.Globalization.CultureInfo.CurrentCulture);
           }
       }
