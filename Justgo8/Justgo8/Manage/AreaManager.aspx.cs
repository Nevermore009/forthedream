    ...
    public partial class AreaManager : System.Web.UI.Page
    {
        ...
        protected void BindTravelType()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                droptraveltype.Items.Insert(0, "请选择区域");
            }
            else
            {
                droptraveltype.DataSource = null;
                droptraveltype.DataBind();
                droptraveltype.Items.Insert(0, "请先添加区域");
            }
        }

        protected void BindArea(int traveltypeid)
        {
            DataTable dt = new DataTable();
            dt = Bll.BArea.AreaOfTravelType(traveltypeid);
            if (dt.Rows.Count > 0)
            {
        ...
        {
            int areaid = int.Parse(droptraveltype.SelectedValue);
            BindArea(areaid);
        }
    }
}