...
namespace SMCShine.Logic
{
    ...
    {
        public static string SMCShineConnectionStr = ConfigurationManager.ConnectionStrings["SMCShine"].ConnectionString;
        ...
        {
            SqlConnection con = new SqlConnection(SMCShineConnectionStr);
            try
            {
                con.Open();
            }
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter(selectCommandText, SMCShineConnectionStr);
            foreach (SqlParameter p in parameters)
        ...
        /// <param name="parameters">存储过程参数,null表示无参数</param>
        /// <returns></returns>
        public static bool ExecuteProcedure(string storedProcName, params SqlParameter[] parameters)
        {
            SqlCommand cmd = GetCommand(parameters);
            try
            {
